...
3. Merge the previous caption and the current caption by clicking "Merge" in between the previous caption and the current caption if you found no puncuation at the end of word on the previous caption. You can use Ctrl-A (or Cmd-A on Mac) on the current caption, then Ctrl-X (or Cmd-X on Mac) to the previous caption. Or you can right click the current caption then click "Select All", right click again the selected text then click "Cut", right click on the previous caption, finally click "Paste".
...
~You can use ASIF Subtitle to check if there are improper single-word captions. This improper single-word caption detection is featured in ASIF Subtitle, so you need not check them one by one, just search which matches.~
